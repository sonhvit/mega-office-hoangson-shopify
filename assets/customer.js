...
class CustomerAddresses {
	...
	_toggleExpanded(target, items) {
		...
		if (target.getAttribute("data-type") === "reset") {
			...
			list.classList.remove("non-visible");
			list.classList.remove("items_non-visible");
			list.querySelectorAll(".address-list__item").forEach((elem) => {
				elem.classList.remove("non-visible");
			});
			item
			...
			if (item.getAttribute(attributes.expandedTarget) !== "false") {
				...
				list.classList.add("non-visible");
			}
			list.classList.remove("items_non-visible");
			list.querySelectorAll(".address-list__item").forEach((elem) => {
				elem.classList.remove("non-visible");
        elem.previousElementSibling.setAttribute("data-expand", "false");
			});
			...
			const list = document.getElementById("addressList");
			list.classList.add("items_non-visible");
			list.querySelectorAll(".address-list__item").forEach((elem) => {
				elem.classList.add("non-visible");
			});
